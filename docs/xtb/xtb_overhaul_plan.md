...
2. **Round all computed monetary outputs to 2dp** (generalizes D11) — apply `round(_, 2)` to every computed monetary value before encryption so the IEEE-754 artifact D11 targets doesn't reappear in the encrypted columns.
...
7. **Warn on sell row with no Closed Positions match** — if a `Stock sell` row's `position_id` has no match in the Closed Positions lookup (partial export), log a warning and leave `fee_amount`/`settle_date` null. Fee analytics are silently incomplete without the signal.
