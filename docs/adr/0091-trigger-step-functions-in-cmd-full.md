...
4. **State machine ARN resolved via boto3.** `resolve_state_machine_arn(sfn_client, mode)` discovers the ARN at runtime using `list_state_machines` and the well-known state machine name, eliminating env vars. Not hardcoded. Missing ARN → actionable error naming the Terraform source. *(Superseded: the original decision used `STAGING_STATE_MACHINE_ARN` / `PROD_STATE_MACHINE_ARN` env vars; ADR 0093 replaced this with boto3 lookup.)*
