...
- The report layer (`charts.py`/`renderer.py`) gets no null handling: gold tables are written with `mode="overwrite"` on every analytics run (ADR 0077) and the raise above fires before any write, so a null-carrying gold table is unreachable — `cash_flow_breakdown` and `passive_income_timeline` sum `target_value` unconditionally.
...
- Handle nulls in the report layer (flag figure or fail the whole report): dead code. Since gold tables are always overwritten and the raise prevents null writes, the report can only ever read null-free gold tables; any report-side null handling would never fire and would silently undercount in the one unreachable case it guarded.
...
- The report layer has no null handling for `target_value`/`target_ccy`: gold tables are overwritten on every analytics run, and the raise prevents null writes, so the report can only read null-free gold tables.
...
- **Negative**: if the report is generated against a gold table written by pre-0111 analytics (before analytics is re-run), null `target_value` rows are silently omitted from sums — not reachable in the normal flow (gold tables are always overwritten and the raise prevents new nulls), so no report-side warning exists for that one window.
...
- `tests/test_charts.py`: `test_prefers_target_value_over_cash_amount` asserts `cash_flow_breakdown` sums `target_value`; the two `test_flags_on_null_target_value` tests were deleted with the flag layer — the report layer has no null-handling code to test.
- Full suite passes; `ruff check --fix`, `ruff format`, and `pyright pipeline/ tests/` clean.
